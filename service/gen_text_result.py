...
from model.linguistic_head import get_tokenizer, get_linguistic_model
...
def get_emotion_features_from_text(tokenized_text):
    ...
    output_valence, output_arousal, output_emo, pen_features = linguistic_model(tokenized_text)
    ...
    pen_features = pen_features.cpu().detach().numpy()

    return output_valence, output_arousal, output_emo, pen_features


async def process_text_file(file_path: str, result_path: str, socket: WebSocket):
    ...
    for (i, segment) in enumerate(input_text["segments"]):
        ...
        linguistic_features = get_emotion_features_from_text(tokenized_text)
        # Get emotion features from text
        ling_valence, ling_arousal, ling_emotion, ling_pen_features = linguistic_features

        # Mapping linguistic outputs to main label space
        main_linguistic_emo_prob = bold_to_main(ling_emotion[0])
        main_linguistic_valence = bold_to_main_valence(ling_valence[0])
        main_linguistic_arousal = bold_to_main_arousal(ling_arousal[0])
